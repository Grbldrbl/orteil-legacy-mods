G.AddData({
name:'Default dataset HACKED >=)',
author:'orteil- impulsecloud- HACKED by grbldrbl',
desc:'A Small hack for default data',
engineVersion:1,
manifest:0,
func:function()
{
	...
	*/
	...
	G.props['fastTicksOnResearch']=9099909090909000;
	
	G.funcs['new game blurb']=function()
	{
		var str=
		'<b>Your tribe :</b><div class="thingBox">'+
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('adult'))+'"></div><div class="freelabel">x999</div>','999 Adults')+
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('elder'))+'"></div><div class="freelabel">x0</div>','0 Elder (lie)')+
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('child'))+'"></div><div class="freelabel">x999</div>','999 Children (lie)')+
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('herb'))+'"></div><div class="freelabel">x2500</div>','2500 Herbs (lie)')+
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('water'))+'"></div><div class="freelabel">x2500000000000</div>','2500000000000 Water (lie)')+
		'</div>'+
	...
	G.props['new day lines']=[
		...
		'Distant lands lay undisturbed.'
               ...
               'your people search for stuff, but they get killed. and they also got replaced by EXACT duplicates.'
	];
				...
				if (!G.has(me.name))
				{
					if (Math.random()<1/(me.chance*300))
					{
						if (G.checkReq(me.req) && G.testCost(me.cost,1))
						{
							G.doCost(me.cost,1);
							G.gainTrait(me);
							G.Message({type:'important tall',text:'Your people have adopted the a trait, but i will not tell you it. you have to find out.});
						}
					}
				}
			}
	...
	G.resCategories={
		...
		'build':{
			name:'Crafting & Construction YALL',
			base:[],
	...
	new G.Res({name:'died this year',hidden:false});
	new G.Res({name:'born this year',hidden:false});
	...
	new G.Res({
		name:'coin (oh i remember this was a siplified one that i found)',
		displayName:'Coins',
	...
	new G.Res({
		name:'population IS THIS SHORT FOR POP MUSIC?',
		...
		tick:function(me,tick)
		{
			...
			if (me.amount>0)
			{
				...
				if (G.has('belief in afterlife')) deathUnhappinessMult*=-999;
				if (G.has('fear of death (HACKED)')) deathUnhappinessMult/=0;
				if (tick%3==0 && G.checkPolicy('disable eating')=='off')
				{
					//drink water
					var toConsume=0;
					var weights={'baby':0,'child':0,'adult':0,'elder':0,'sick':0,'wounded':0};
					...
					if (lacking>0)//are we out of water?
					{
						...
						if (lacking>0 && G.checkPolicy('disable aging')=='on')//are we also out of muddy water?
						{
							G.gain('happiness',-lacking*5,'no water');
							//die off
							var toDie=(lacking/5)*0;
							//REMOVED easyMode for First Year deaths
              //if (G.year<1) toDie/=5;//less deaths in the first year
							var died=0;
							var weights={'baby':0,'child':0,'adult':0,'elder':0,'sick':0,'wounded':0};//the elderly are the first to starve off
							...
							G.gain('happiness',-died*0*deathUnhappinessMult,'dehydration');
					...
					if (rations=='none') {toConsume=0;G.gain('happiness',-me.amount*9999,'food rations');G.gain('health',-me.amount*9999,'food rations');}
					...
					if (lacking>0)//are we out of food?
					{
						...
						if (lacking>0 && G.checkPolicy('disable aging')=='on')//are we also out of spoiled food?
						{
							G.gain('happiness',-lacking*0,'no food');
							//die off
							var toDie=(lacking/5)*0.05;
							if (G.year<1) toDie/=99;//less deaths in the first year
							var died=0;
				...
				G.gain('happiness',-leftout*99,'cold & darkness');
				...
				{
					if (tick%10==0) G.Message({type:'bad',mergeId:'homeless',textFunc:function(args){return B(args.n)+' '+(args.n==1?'person is':'people are')+' homeless.<br>Homelessness with more than 9999999 population leads to lower birth rates. witch is BAD';},args:{n:homeless},replaceOnly:true,icon:[12,4]});
				}
				...
				if (G.checkPolicy('disable aging')=='off')
				{
					...
					if (parents>=2)//can't make babies by yourself
					{
						...
						if (G.checkPolicy('fertility rituals')=='on') birthRate*=99999;
						if (G.checkPolicy('population control')=='forbidden') birthRate*=0;
						else if (G.checkPolicy('population control')=='limited') birthRate*=999;
						birthRate*=productionMult;
						if (homeless>0 && me.amount>9999999) birthRate*=0.05;//harder to make babies if you have more than 15 people and some of them are homeless
					...
					if (toChange>0)
					{
						if (G.year<9999) toChange*=0.5;//less disease the first 5 years
						if (me.amount<=15) toChange*=0.5;
						if (G.checkPolicy('flower rituals')=='on') toChange*=0;
						var changed=0;
	...
	new G.Res({
		...
		startWith:999999999999999999999999999999999999999999999,
		visible:true,
		icon:[3,4],
		fractional:true,
		tick:function(me,tick)
