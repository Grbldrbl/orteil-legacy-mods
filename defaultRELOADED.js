...
func:function()
{
	...
	*/
	...
	G.props['fastTicksOnResearch']=50;
	
	G.funcs['new game blurb']=function()
	{
		var str=
		'<b>Your default tribe :</b><div class="thingBox">'+
		...
		'</div>'+
		'<div class="par fancyText bitBiggerText">Your tribe finds a place to settle in the wilderness.<br>Resources are not that scarce, but  everyone starts foraging still.</div>'+
		'<div class="par fancyText bitBiggerText">You emerge as your tribe\'s leader. They call you :</div>';
		return str;
	}
	
	G.funcs['new game']=function()
	{
		var str='Your name is '+G.getName('ruler')+''+(G.getName('ruler').toLowerCase()=='grbldrbl'?' <i><3</i>':'')+', ruler of '+G.getName('civ')+'. Your tribe is primitive, but full of hope.<br>The first year of your legacy has begun. May it stand the test of time.';
    ...
    'your people start wondering about the future.'
	];
	...
	G.funcs['civ blurb']=function()
	{
		...
		if (pop>0)
		{
			...
			var stat=G.getRes('happiness').amount/12;
			...
			if (me.amount>0)
			{
				...
				if (tick%3==0 && G.checkPolicy('disable eating')=='off')
				{
					...
					if (lacking>0)//are we out of water?
					{
						...
						{
							G.gain('happiness',-lacking*0.5,'no water');
							//die off
							var toDie=(lacking/5)*0.01;
				...
				if (G.checkPolicy('disable aging')=='off')
				{
					...
					if (G.year>=10)//no aging adults the first 10 years
					{
	...
	new G.Unit({
		...
		effects:[
			...
			{type:'gather',context:'gather',what:{'water':12,'muddy water':0},amount:1,max:3},
		...
		req:{'tribalism':true, 'speech':true},
		category:'production',
		priority:10,
	});
	...
	new G.Unit({
		...
		req:{'speech':true, 'tribalism':true},
		category:'discovery',
		priority:5,
	});
